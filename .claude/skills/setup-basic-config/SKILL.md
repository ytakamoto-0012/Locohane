...
**対象外**: 上記7項目以外（`config.ini` の timeout系 `[llm].request_timeout_seconds` 等は
...
1. `config.ini`・`app.bat`・プロジェクト直下 `CLAUDE.md` を読み、上記7項目の現在値をユーザーに提示する。
2. 各項目について「現在値のままでよいか、新しい値にするか」を対話で尋ねる。
   ...
   - 7項目まとめて一度に聞いても、ファイル単位・1つずつ聞いてもよい。ユーザーが
     答えやすい形で進める。
