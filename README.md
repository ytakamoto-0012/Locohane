...
- 1つの会話（thread_id）につき1ファイルへ追記し続ける（日をまたいでも
  ファイルは変わらない）。左サイドバーからスレッドを再開した場合も、同じ
  thread_id の既存ファイルへ追記を継続する（新規ファイルには分割しない）。
  再開したタイミングは `----- 再開 YYYY-MM-DD HH:MM:SS -----` という区切り行で
  ファイル内に記録される。
