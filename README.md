...
- **メインエージェントの1リクエストあたりトークン量を上限内に抑える設計**
  ...
  多重に補強している。
- **入力文字数の事前ガード**（`src/input_length_guard.py`、`app.py` の
  `on_message`、`config.ini` の `[graph] input_length_guard_*`）:
  ユーザーが1ターンで送信した生入力テキストの文字数を、LLM呼び出し前に
  事前チェックする。閾値（既定64,000文字）を超えていてもLLM呼び出し自体は
  止めず、「一度に処理しようとせず段階を踏んで分割して進めてください」
  という注意書きを本文の先頭に追加してから渡す。`main_token_guard`
  （LLM応答後の `total_tokens` をReActループ内で監視する事後対応）とは
  判定材料・発火タイミングが異なり、送信前の一次防波堤として機能する。
