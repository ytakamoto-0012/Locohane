...
含めること（本番インシデント: 2026-08-14, 75行の表のうち代表例として挙がった
14行だけを「問題箇所」として報告し、それ以外の未列挙の行に残っていた同種の
不整合が後続の修正作業から漏れ落ちた）。件数が多い場合は目視サンプリングに
頼らず、`json_query`/`Grep`で取得した値を規則と突き合わせるロジックを
自分で組み立てて全件処理する。
...
5. 表・繰り返し構造の異常を探す依頼では、代表例だけで終えず全行・全項目を
   ...
   `execute_python_code_readonly`で実際に算出した値と突き合わせる（ルール7）。
