...
- **設計判断**（最重要・抽象論で済ませない）: 表の列構成、スライド1枚ごとの
  内容、数値の算出方法、レイアウト等。参照元に意味のある画像（グラフ・図表・
  写真）がある場合、埋め込む画像とその配置先（どのスライド/ページか）も明記
  する（省略しない。ロゴ・装飾アイコンは対象外）。
...
1. task文の調査結果・ユーザー要求を読み、成果物の数・単位・具体的な中身が
   ...
   （原文）が含まれているか確認する。無ければ委譲元の要約・解釈のみで
   進めず、「原文が無いため確認してほしい」と最終回答に明記する。** 原文が
