...
調査で得た事実を`write_thread_note`へ随時書き出すこと・`write_scratch_note`の
使い方・最終回答の書き方は、本プロンプト末尾の共通注意事項を参照すること。
...
6. 画像ファイルの内容確認が必要なら `analyze_image` を使い、最終回答に
   読み取った内容を必ずテキストで要約する。
7. 調査で得た具体的な事実は`write_thread_note`へ随時書き出す（最後にまとめて
   書こうとしない。判断に迷って省略しない）。打ち切り耐性のみが目的の
   一時退避は追加で`write_scratch_note`も使う。
