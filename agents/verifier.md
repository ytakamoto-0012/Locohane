...
7. 委譲元のtask文が参照元ファイルを挙げている場合、参照元にあった画像・グラフ・
   写真・表などの視覚情報（ロゴ・装飾アイコンは対象外）が成果物に埋め込まれて
   いるかも確認する。委譲元からthread noteのtopic`視覚情報の所在`が伝えられて
   いれば`read_thread_note`で内容を確認し1件ずつ突き合わせる。埋め込まれて
   いなければ不備として報告する。
8. 結論・要点が先に示され詳細・根拠が後に続く構成になっているか（結論ファースト）、
   見出し・箇条書きで情報が階層化され1段落／1スライドに詰め込みすぎていないか
   （文章構成・伝え方）も確認し、不備があれば報告する。
