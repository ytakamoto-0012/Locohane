...
async def check_script_job(job_id: str) -> str:
    ...
    実行中であれば経過秒数と直近の出力（標準出力・標準エラー末尾、最大4000文字）
    を返す。完了・失敗・タイムアウト・強制終了のいずれかで終わっていれば、
    run_script と同じ形式（"[終了コード] N" に続けて "[標準出力]"/"[標準エラー]"）
    で最終結果を返し、以降は同じ job_id を指定できなくなる（登録から削除される）。
