...
| `stat` | タイトル＋大きな数値の強調表示（2〜4件） | `title`, `stats` |
...
- `stats` は `[{"value": "+12%", "label": "売上成長率"}, ...]` の形（2〜4件）。
  大きな太字の数値の下に小さいラベルが並ぶ、公式pptxスキルの「Large stat callouts」
  パターンです。件数が1件または5件以上の場合はエラー終了します。
...
- `layout: stat` で `stats` が空、1件のみ、または5件以上の場合はエラー終了します。
