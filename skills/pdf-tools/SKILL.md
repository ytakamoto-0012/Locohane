...
- 日本語はreportlab内蔵のCIDフォント（`HeiseiMin-W3`=明朝本文, `HeiseiKakuGo-W5`=ゴシック見出し）
  で描画されます。外部フォントファイルは不要です。ただしこの方式はグリフをPDFへ
  埋め込まず、閲覧側（Acrobat等のビューア）が環境のCJKフォントで代替表示する仕組み
  のため、ビューアによって字形が微妙に異なることがあります。

## エッジケース共通

- 依存パッケージ `pypdf`（テキスト抽出）・`pypdfium2`（画像化）・`xhtml2pdf`（生成）が
  実行環境に無いと `ModuleNotFoundError` で終了コード非0になります。その場合は導入者へ
  `pip install pypdf pypdfium2 xhtml2pdf` の実施を促してください。
