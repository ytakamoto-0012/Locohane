...
- 一部のページのみ内部的に破損している場合、そのページだけ `text: ""` と
  `error` キー（失敗理由）が入り、他のページは通常通り抽出されます
  （1ページの破損で全体が失敗することはありません）。
...
本文HTMLファイルは基本的にUTF-8で保存してください）。
`--title` を指定するとPDFのメタデータ（タイトル）に設定されます（省略可）。
