...
2. **回答本文の構造（一覧表など）の一部として画像を組み込みたい場合**:
   ...
   `src/images.py` の `embed_local_images_as_data_urls()` を通し、実在する
   画像ファイルを指す `![...](絶対パス)` だけを自動で `data:` URL に変換
   してからブラウザへ送る（ローカル絶対パスのままではブラウザは画像を
   読み込めないため）。
...
- 方式2で埋め込まれるのは**縮小済みのサムネイル**（既定 長辺320px・JPEG品質70、
  ...
  （Chainlitの送信経路には1メッセージ既定1MBの上限があり、フル解像度のまま
  複数枚埋め込むと上限超過でメッセージ自体が送信できなくなるため）。
  細部を確認させたい／高解像度で見せたい場合は方式2ではなく `analyze_image`
  （LLM自身が解析する場合）または `show_image`（独立メッセージとして原寸で
  見せる場合）を使わせること。
