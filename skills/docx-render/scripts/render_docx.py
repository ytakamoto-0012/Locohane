...
run_script ツールから
    python render_docx.py <docx_path> [--start-page N] [--max-pages N] [--dpi N] [--crop]
の形で呼ばれる。
...
既定はクロップなし（ページの実際の端がそのまま見える）。用紙サイズ・余白は
docxファイル自体が定義済みで、ピクセル走査で推測する必要が無いうえ、
クロップすると余白の均等さ・中央揃え等のレイアウトQAに必要な情報が失われる
ため。表計算のような「意味のあるデータ領域が可変」なexcel-renderとは
事情が異なる。
...
def main() -> int:
    ...
    for image in result.get("images", []):
        pm = register_output_path(image["image_path"],
                                  description=f"render_docxが生成したページ{image['page']}の画像")
        if pm:
