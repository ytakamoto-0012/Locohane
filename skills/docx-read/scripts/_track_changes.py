...
run_script からは直接実行されない。read_docx.py から import して使う。
実体は skills/docx-edit/scripts/_track_changes.py と同一（read_docx.py側は
count_revisions のみ使用するが、スキル間の相互importを避けるため
全文を複製している。変更する場合は両方に同じ内容を反映すること）。
"""
