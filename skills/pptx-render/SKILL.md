...
| `--crop`（値なしフラグ） | 任意 | - | 付けない＝クロップなし | 付けるとコンテンツ領域のbboxで余白を除去する（後述） |
...
```
`--crop`を付けたときのみ、`images`の各要素に`"cropped": true/false`が追加される
（コンテンツ領域のbbox検出に成功した画像のみ`true`。検出できなかった画像は
`false`で元画像のまま）。`--crop`を付けない場合、`cropped`キー自体が存在しない。
